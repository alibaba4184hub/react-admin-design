...
import { shopTypeDict, authStatusDict, shopStatusDict } from '@/constant/storeManage/dict'
...
const SheinManage: React.FC = () => {
  ...
  const [tableQuery, setTableQuery] = useState<PageState>({ current: 1, pageSize: 10, salePlatformId: 30 })
  ...
  const columns: ColumnsType<TableDataType> = [
    ...
    {
      title: '授权到期时间',
      dataIndex: 'tkRefreshTokenExpireIn',
      align: 'center',
      render: (_, record: any) => <span>{record.tkRefreshTokenExpireIn || '--'}</span>
    },
    {
      title: '操作用户',
      dataIndex: 'operatorUser',
      align: 'center',
      render: (_, record: any) => <span>{record.operatorUser || '--'}</span>
    },
    {
      title: '操作',
      key: 'action',
